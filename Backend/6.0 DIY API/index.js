...
app.get(`/jokes/:id`, (req, res)=>{
  ...
  const jokeFound = jokes.find((joke)=> joke.id === idInURL);
...
app.get("/filter", (req, res)=>{
  const typeInURL = req.query.type;
  const typeInJoke = jokes.filter((joke)=> joke.jokeType === typeInURL);
  // console.log(typeInJoke);
  res.json(typeInJoke);
});
...
app.put("/jokes/:id", (req, res)=>{
  ...
  const searchIndex = jokes.findIndex((joke)=> joke.id === idInURL);
...
app.patch("/jokes/:id", (req, res)=>{
  const idInURL = parseInt(req.params.id);
  const existingJoke = jokes.find((joke)=> joke.id === idInURL);
...
app.delete("/all", (req, res)=>{
  ...
  if(userKey === masterKey){
    ...
    res.send(404).json({error : `You are not authorised to perform this action.`});
  }
})
